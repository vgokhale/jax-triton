...
def mha(q, k, v,
  ...
  block_q = 128 #min(block_q, seq_len)
...
def _preprocess_backward(out, do, block_q: int,
                         debug: bool, interpret: bool):
  batch_size, seq_len, num_heads, head_dim = out.shape
  out_shape = [
      jax.ShapeDtypeStruct(do.shape, do.dtype),
      jax.ShapeDtypeStruct(shape=(batch_size, num_heads, seq_len), # delta 
                           dtype=jnp.float32),
  ]
...
def _mha_backward(sm_scale: float, causal: bool, block_q: int, block_k: int,
  ...
  block_q = min(block_q, seq_len)
  block_k = min(block_k, seq_len)
  do_scaled, delta = _preprocess_backward(out, do, block_q, debug, interpret)
  ...
  if backward_pass_impl == "xla":
    ...
    # We accumulate into dq so we need to initialize it to zeros.
    out_shapes_q = jax.ShapeDtypeStruct(q.shape, q.dtype)
    ...
    block_q = 64
    grid_kv = (1, batch_size * num_heads, jt.cdiv(seq_len, block_k))
    out_shapes_kv = [
      jax.ShapeDtypeStruct(k.shape, k.dtype),
      jax.ShapeDtypeStruct(v.shape, v.dtype),
    ]
